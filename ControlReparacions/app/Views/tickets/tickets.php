...
<div>
    ...
    <?php if ((session()->get('user')['role'] == "sstt") || (session()->get('user')['role'] == "ins")  || (session()->get('user')['role'] == "prof") || (session()->get('user')['role'] == "admin")) : ?>
      ...
      <div class="relative">
        ...
        <ul class="rounded-lg flex flex-col shadow-lg border border-primario text-base text-terciario-4 " aria-labelledby="dropdownDefaultButton">
            ...
            <!-- Export CSV  -->
            <li>
              <a href="<?= base_url('tickets/export/csv?q=' . $search . "&d=" . $filters['device'] . "&c=" . $filters['center'] . "&dt_1=" . $filters['date_ini'] . "&dt_2=" . $filters['date_end'] . "&tm_1=" . $filters['time_ini'] . "&tm_2=" . $filters['time_end'] . "&e=" . $filters['state']) ?>" class="rounded-t-md transition hover:ease-in ease-out duration-250 block px-4 py-2 hover:bg-primario hover:text-secundario">
            ...
            <!-- Export XLS -->
            <!-- <li>
              <a href="<?= base_url('tickets/export/xls?q=' . $search . "&d=" . $filters['device'] . "&c=" . $filters['center'] . "&dt_1=" . $filters['date_ini'] . "&dt_2=" . $filters['date_end'] . "&tm_1=" . $filters['time_ini'] . "&tm_2=" . $filters['time_end'] . "&e=" . $filters['state']) ?>" class="rounded-t-md transition hover:ease-in ease-out duration-250 block px-4 py-1 hover:bg-primario hover:text-secundario">
            ...
            <!-- <li>
              <form action="<?= base_url('tickets/import/xls') ?>" method="POST" enctype="multipart/form-data">
                <label for="uploadXLS" class="rounded-t-md transition hover:ease-in ease-out duration-250 block px-4 py-1 hover:bg-primario hover:text-secundario">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 mr-2 fill-current inline" viewBox="0 0 32 32">
                    <path d="M23.75 11.044a7.99 7.99 0 0 0-15.5-.009A8 8 0 0 0 9 27h3a1 1 0 0 0 0-2H9a6 6 0 0 1-.035-12 1.038 1.038 0 0 0 1.1-.854 5.991 5.991 0 0 1 11.862 0A1.08 1.08 0 0 0 23 13a6 6 0 0 1 0 12h-3a1 1 0 0 0 0 2h3a8 8 0 0 0 .75-15.956z" data-original="#000000" />
                    <path d="M20.293 19.707a1 1 0 0 0 1.414-1.414l-5-5a1 1 0 0 0-1.414 0l-5 5a1 1 0 0 0 1.414 1.414L15 16.414V29a1 1 0 0 0 2 0V16.414z" data-original="#000000" />
                  </svg>
            ...
            <!-- Imprimir PDF  -->
            <li>
              <a href="<?= base_url('TicketsPDF?q=' . $search . "&d=" . $filters['device'] . "&c=" . $filters['center'] . "&dt_1=" . $filters['date_ini'] . "&dt_2=" . $filters['date_end'] . "&tm_1=" . $filters['time_ini'] . "&tm_2=" . $filters['time_end'] . "&e=" . $filters['state']) ?>" class="block px-4 py-2 hover:bg-primario hover:text-secundario transition hover:ease-in ease-out duration-250">Imprimir PDF</a>
            </li>

            <!-- Etiquetes PDF  -->
            <li>
              <a href="<?= base_url('labels?q=' . $search . "&d=" . $filters['device'] . "&c=" . $filters['center'] . "&dt_1=" . $filters['date_ini'] . "&dt_2=" . $filters['date_end'] . "&tm_1=" . $filters['time_ini'] . "&tm_2=" . $filters['time_end'] . "&e=" . $filters['state']) ?>" class="block px-4 py-2 hover:bg-primario hover:text-secundario transition hover:ease-in ease-out duration-250">Etiquetes PDF</a>
            </li>

            <!-- Plantilla CSV  -->
            <li>
              <a href="<?= base_url('tickets/dowloadCSV') ?>" class="block px-4 py-2 hover:bg-primario hover:text-secundario transition hover:ease-in ease-out duration-250 rounded-b-md">Plantilla CSV</a>
            </li>
  ...
  <form id="filters_form" action="<?= base_url('tickets') ?>" method="get" class="hidden relative gap-4 items-center mb-2 mt-2 border-y-2 p-3">
    ...
    <div class="absolute right-2">
      <button onclick='document.getElementById("filters_form").submit();' class="bg-primario text-white text-base font-semibold px-8 py-1 border border-terciario-4 rounded-lg hover:bg-terciario-4 transition hover:ease-in ease-out duration-250"><?= lang('buttons.filter') ?></button>
      <a href="<?= base_url('tickets') ?>" class="bg-primario text-white text-base px-8 py-1.5 border font-semibold border-terciario-4 rounded-lg hover:bg-red-500 transition hover:ease-in ease-out duration-250"><?= lang('buttons.clear') ?></a>
    </div>
