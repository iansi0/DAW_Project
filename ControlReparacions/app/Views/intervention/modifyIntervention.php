...
<div style="view-transition-name: addTicket;" class="container mx-auto px-4 py-8 text-base">
    ...
    <div>
        <div class="flex">
            ...
            <h1 class="text-5xl text-primario"><?= mb_strtoupper(lang("titles.e_int")) ?></h1>
    ...
    <form action="modify" method="POST" class="mt-4 grid grid-cols-12 px-20">
        ...
        <div class="shadow-lg border-b border-primario rounded-t-xl mb-4 pb-2 col-span-12">
            ...
            <div class="grid grid-cols-12">
                <div class="col-span-8" style="position: relative;">
                    ...
                    <textarea id="description" maxlength="100" oninput="sumChar()" name="description" class="border-2 border-terciario-1 h-full w-full pt-5 ps-2 rounded hover:bg-secundario transition hover:ease-in ease-out duration-150"><?=$intervention['descripcio']?></textarea>
                </div>
...
<script>
    ...
    function addLine(value) {
        ...
        <?php foreach ($inventaryNoAssigned as $type): ?>
        ...
        <?php foreach ($inventaryAssigned as $type): ?>
            option = document.createElement('option');
            option.value = '<?= $type["id"] ?>';
            if (value == '<?= $type["id"] ?>') {
